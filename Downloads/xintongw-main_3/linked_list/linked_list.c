...
extern void ll_splice(linked_list_t *list1, linked_list_t *list2, 
   ...
   assert(list2);
    ...
    if(overlap!=ll_size(list2)){
        ll_node_t *p1=list2->head;//if list1 does not contain list2,set pointer to traverse linked list
        
        for(int i=0;i<overlap;i++){
            p1=p1->next;
        }//find the next node after list2 overlaps with list2
